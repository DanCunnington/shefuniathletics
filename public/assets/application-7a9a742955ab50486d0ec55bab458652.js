...
var ready = function() {
  ...
  var appendCoachesContentsInterval =  setInterval(function() {

    //Get current value
    var currentText = $("#coachHiddenContent").val();
    ...
    if (currentText) {
      if ($(".cke_wysiwyg_frame").contents().find("body").get(0)) {
        ...
        if ((contentsOfFrame == "undefined") || (contentsOfFrame == ""))  {
          ...
          return clearInterval(appendCoachesContentsInterval);
        }
      }
    } else {
      return clearInterval(appendCoachesContentsInterval);
...
ready = function(){
    // call sortable on our div with the sortable class
    $('.records-sortable').sortable();

    // after the order changes
    $('.records-sortable').sortable().bind('sortupdate', function(e, ui) {
        // array to store new order
        updated_order = []
        // set the updated positions
        set_positions();
